...
Element.prototype.matchesElement = function(element) {
    return this.tagNames[element.tagName] &&
        (isStructuralListPlaceholder(this) ||
            isStructuralListItemPlaceholder(this) ||
            _.isEqual(this.attributes || {}, element.attributes || {}));
};

function isStructuralListPlaceholder(element) {
    return element.tagNames.ul && element.tagNames.ol &&
        Object.keys(element.tagNames).length === 2 &&
        _.isEmpty(element.attributes || {});
}

function isStructuralListItemPlaceholder(element) {
    return element.tagNames.li &&
